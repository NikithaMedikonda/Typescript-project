...
keyEvent.addEventListener('keyup', function (event: KeyboardEvent) {
    if (event.key !== 'Enter') return;

    const description: string = (event.target as HTMLInputElement).value.trim();
    (event.target as HTMLInputElement).value = "";
    (event.target as HTMLInputElement).placeholder = "Enter item";
    ...
    const itemExists = itemList.some(item => item.description.toLowerCase() === description.toLowerCase() && !item.isDeleted);
    if (itemExists) {
